    ...
    if (
      ...
      const newGov = goverments.map((gov) => {
        if (gov.id === goverment.id) {
          ...
          gov.municipalityName = dropDown2.filter(
            (item) => item.id === +dropDown2Ref.current.value
          )[0].name;
        }
      ...
      if (!goverment.new) {
        const send = fetch(
          `https://marsad.almofawter.net/api/Centers/${goverment.id}`,
          {
            method: "PUT",
            headers: {
            ...
            body: JSON.stringify({
              name: goverment.name,
              municipalityId: +dropDown2Ref.current.value,
            }),
          }
        )
          .then((res) => toast.success("تم التعديل", { closeOnClick: true }))
          .catch((err) =>
  ...
  return (
    <>
      <tr>
        ...
        <td>
          {edit || !goverment.name ? (
            <select name="cars" ref={dropDown2Ref}>
              <option defaultValue="0" disabled selected>
                اختار البلدية
              </option>
              {dropDown2.map((item, ind) => (
